...
//      .toSet
//
//  override def setSingleProperty[D, G](element: Element, key: String, value: D, mapping: SingleMapping[D, G]): Unit =
//    mapping.reverse(value).foreach(element.property(key, _))
//
...
object Binary {
  val model: Model.Vertex[Binary] = new VertexModel {
    ...
    override def create(binary: Binary)(implicit db: Database, graph: Graph): Vertex = {
      ...
      UniMapping.binary.setProperty(v, "data", binary.data)
      UniMapping.string.optional.setProperty(v, "folder", binary.folder)
      UniMapping.string.optional.setProperty(v, "attachmentId", binary.attachmentId)
      v
    }

    override val converter: Converter[Binary with Entity, Vertex] = (element: Vertex) => {
      val data         = UniMapping.binary.getProperty(element, "data")
      val folder       = UniMapping.string.optional.getProperty(element, "folder")
      val attachmentId = UniMapping.string.optional.getProperty(element, "attachmentId")
...
object BinaryLink {
  lazy val model: Model.Edge[BinaryLink, Binary, Binary] = new EdgeModel[Binary, Binary] {
