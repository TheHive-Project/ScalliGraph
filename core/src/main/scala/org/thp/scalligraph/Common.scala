...
class RichFuture[T](future: Future[T]) {
  ...
  def await(implicit duration: Duration = 10 seconds span): T =
...
class RichJson(obj: JsObject) {
  ...
  def collect(pf: PartialFunction[(String, JsValue), (String, JsValue)]) =
    JsObject(obj.fields.collect(pf))
}
