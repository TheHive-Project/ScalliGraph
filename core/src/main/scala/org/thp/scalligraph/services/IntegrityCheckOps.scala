...
trait IntegrityCheckOps[E <: Product] extends IntegrityCheck with MapMerger with TraversalOps {
...
trait DedupCheck[E <: Product] extends IntegrityCheck with IntegrityCheckOps[E] {
  ...
  def getDuplicates[A](properties: Seq[String], killSwitch: KillSwitch): Seq[Seq[E with Entity]] =
    if (properties.isEmpty) Nil
      ...
      val bloomFilter = BloomFilter[Vertex](math.max(numberOfElements, 1L), 0.01)
...
trait GlobalCheck[E <: Product] extends IntegrityCheck with MapMerger with TraversalOps {
  override type ENTITY = E
